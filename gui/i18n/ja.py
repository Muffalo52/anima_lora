...
STRINGS: dict[str, str] = {
    ...
    # Soup パイプラインタブ (gui/tabs/soup_tab.py)
    "soup_target": "対象アーティスト",
    "soup_target_tip": "スープ化したい LoRA の対象アーティストのディレクトリ（post_image_dataset/resized/ 配下）。必須です。",
    "soup_pool": "プール（任意）",
    "soup_pool_tip": "uncond インタートレイン用プールのアーティスト名（スペース区切り）。空欄なら対象を含むシャードを使用します。",
    "soup_pool_shard_n": "プールシャードサイズ",
    "soup_pool_shard_n_tip": "1 つの uncond 初期化チェックポイントを共有するアーティスト数（デフォルト 16）。",
    "soup_uncond_ratio": "Uncond 比率",
    "soup_uncond_ratio_tip": "uncond インタートレイン段階の caption_dropout_rate（デフォルト 0.5）。",
    "soup_uncond_epochs": "Uncond エポック",
    "soup_uncond_epochs_tip": "uncond インタートレイン段階のエポック数（デフォルト 2）。",
    "soup_seeds": "シード",
    "soup_seeds_tip": "スペース区切りのシード。シードごとにファインチューニングを 1 回ずつ行い、スープ化します（デフォルト 1001 1002 1003）。",
    "soup_rank": "SVD ランク",
    "soup_rank_tip": "スープ ΔW を SVD で切り詰めるランク。0 ならメソッドの network_dim を使用します。",
    "soup_ft_args": "ファインチューニング引数",
    "soup_ft_args_tip": "各ファインチューニング実行に渡す追加 CLI 引数。例: --network_dim 32 --max_train_epochs 8。",
    "soup_target_required": "Soup には対象アーティストが必要です（post_image_dataset/resized/ 配下のディレクトリ）。",
    "n_images": "{n} 枚の画像",
    # ImageViewerTab
    "directory": "ディレクトリ:",
