...
STRINGS: dict[str, str] = {
    ...
    # Soup 流水线标签页 (gui/tabs/soup_tab.py)
    "soup_target": "目标画师",
    "soup_target_tip": "要做汤（soup）的 LoRA 目标画师目录（位于 post_image_dataset/resized/ 下）。必填。",
    "soup_pool": "池（可选）",
    "soup_pool_tip": "用于 uncond 互训池的画师名称（空格分隔）。留空则使用包含目标的分片。",
    "soup_pool_shard_n": "池分片大小",
    "soup_pool_shard_n_tip": "共享同一个 uncond 初始化检查点的画师数量（默认 16）。",
    "soup_uncond_ratio": "Uncond 比例",
    "soup_uncond_ratio_tip": "uncond 互训阶段的 caption_dropout_rate（默认 0.5）。",
    "soup_uncond_epochs": "Uncond 轮数",
    "soup_uncond_epochs_tip": "uncond 互训阶段的训练轮数（默认 2）。",
    "soup_seeds": "随机种子",
    "soup_seeds_tip": "空格分隔的种子；每个种子做一次微调，然后做成汤（默认 1001 1002 1003）。",
    "soup_rank": "SVD 秩",
    "soup_rank_tip": "汤 ΔW 做 SVD 截断到的秩。0 表示使用方法的 network_dim。",
    "soup_ft_args": "微调参数",
    "soup_ft_args_tip": "转发给每次微调运行的额外 CLI 参数，例如 --network_dim 32 --max_train_epochs 8。",
    "soup_target_required": "Soup 需要一个目标画师（post_image_dataset/resized/ 下的目录）。",
    "n_images": "{n} 张图像",
    # ImageViewerTab
    "directory": "目录:",
    "dataset_reload": "重新加载",
