...
STRINGS: dict[str, str] = {
    ...
    # Soup 파이프라인 탭 (gui/tabs/soup_tab.py)
    "soup_target": "대상 작가",
    "soup_target_tip": "수프로 만들 LoRA의 대상 작가 디렉터리(post_image_dataset/resized/ 아래). 필수 항목입니다.",
    "soup_pool": "풀 (선택)",
    "soup_pool_tip": "uncond 인터-트레인 풀에 쓸 작가 이름(공백으로 구분). 비워두면 대상이 포함된 샤드를 사용합니다.",
    "soup_pool_shard_n": "풀 샤드 크기",
    "soup_pool_shard_n_tip": "하나의 uncond 초기화 체크포인트를 공유하는 작가 수(기본값 16).",
    "soup_uncond_ratio": "Uncond 비율",
    "soup_uncond_ratio_tip": "uncond 인터-트레인 단계의 caption_dropout_rate(기본값 0.5).",
    "soup_uncond_epochs": "Uncond 에폭",
    "soup_uncond_epochs_tip": "uncond 인터-트레인 단계의 에폭 수(기본값 2).",
    "soup_seeds": "시드",
    "soup_seeds_tip": "공백으로 구분한 시드. 시드마다 파인튜닝을 하나씩 돌린 뒤 수프로 합칩니다(기본값 1001 1002 1003).",
    "soup_rank": "SVD 랭크",
    "soup_rank_tip": "수프 ΔW를 SVD로 잘라낼 랭크. 0이면 메서드의 network_dim을 사용합니다.",
    "soup_ft_args": "파인튜닝 인자",
    "soup_ft_args_tip": "각 파인튜닝 실행에 전달할 추가 CLI 인자. 예: --network_dim 32 --max_train_epochs 8.",
    "soup_target_required": "Soup에는 대상 작가가 필요합니다(post_image_dataset/resized/ 아래 디렉터리).",
    "n_images": "이미지 {n}개",
    # ImageViewerTab
    "directory": "디렉토리:",
    "dataset_reload": "새로고침",
