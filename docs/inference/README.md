...
| [foveated.md](foveated.md) | Deferred-foveated merge — full grid above σ_c, then fovea tokens 1:1 + 2×2 periphery token groups merged (endogenous `combo` mask). Identity-preserving, ×1.37 e2e. Runner in `networks/foveated.py`. | `--fovea_sigma_c 0.75` | Euler-only; σ_c=0.75 and the final bicubic readout are load-bearing; frac floor 0.25. Foveated-Spectrum compose CLOSED by `bench/foveated/` P3 — don't re-propose. |
