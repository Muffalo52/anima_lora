...
- `scripts/preprocess/cache_pe_encoder.py` — entry point for `make preprocess-pe` (`post_image_dataset/resized/` → `post_image_dataset/lora/`). Writes `{stem}_anima_{encoder}.safetensors` sidecars consumed by IP-Adapter.
